...
class format_tabbedtopcoll_renderer extends format_section_renderer_base {
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        $this->courserenderer = $this->page->get_renderer('format_tabbedtopcoll', 'course');
        $this->togglelib = new \format_tabbedtopcoll\togglelib;
        $this->courseformat = course_get_format($page->course); // Needed for collapsed topics settings retrieval.

        /* Since format_tabbedtopcoll_renderer::section_edit_control_items() only displays the 'Set current section' control when editing
        ...
        $this->tctoggleiconsize = clean_param(get_config('format_tabbedtopcoll', 'defaulttoggleiconsize'), PARAM_TEXT);
        $this->formatresponsive = get_config('format_tabbedtopcoll', 'formatresponsive');
        ...
        if ($section->section != 0) {
            ...
            if (!empty($controls)) {
                ...
                if (($this->tcsettings['layoutstructure'] == 1) || ($this->tcsettings['layoutstructure'] == 4)) {
                    $topictext = get_string('setlayoutstructuretopic', 'format_tabbedtopcoll');
                } else if (($this->tcsettings['layoutstructure'] == 2) || ($this->tcsettings['layoutstructure'] == 3)) {
                    $topictext = get_string('setlayoutstructureweek', 'format_tabbedtopcoll');
                } else {
                    $topictext = get_string('setlayoutstructureday', 'format_tabbedtopcoll');
                }
                $title = get_string('viewonly', 'format_tabbedtopcoll', array('sectionname' => $topictext.' '.$section->section));
                switch ($this->tcsettings['layoutelement']) { // Toggle section x.
                    ...
                    default:
                        $o .= html_writer::link($url,
                            $this->output->pix_icon('one_section', $title, 'format_tabbedtopcoll'),
    ...
    protected function section_edit_control_items($course, $section, $onsectionpage = false) {
        ...
        if ((($this->tcsettings['layoutstructure'] == 1) || ($this->tcsettings['layoutstructure'] == 4)) &&
            ...
            if ($course->marker == $section->section) {  // Show the "light globe" on/off.
                $url->param('marker', 0);
                $markedthissection = get_string('markedthissection', 'format_tabbedtopcoll');
                ...
                $markthissection = get_string('markthissection', 'format_tabbedtopcoll');
    ...
    protected function section_summary($section, $course, $mods) {
        ...
        $o = '';
        $title = $this->courseformat->get_tabbedtopcoll_section_name($course, $section, false);
        $liattributes = array(
    ...
    protected function section_header($section, $course, $onsectionpage, $sectionreturn = null) {
        ...
        $liattributes = array(
            ...
            'role' => 'region',
            'aria-label' => $this->courseformat->get_tabbedtopcoll_section_name($course, $section, false)
        );
        if (($this->formatresponsive) && ($this->tcsettings['layoutcolumnorientation'] == 2)) { // Horizontal column layout.
            $liattributes['style'] = 'width: ' . $this->tccolumnwidth . '%;';
        }
        ...
        if (($onsectionpage == false) && ($section->section != 0)) {
            ...
            if ($this->userisediting) {
                ...
                $title = $this->courseformat->get_tabbedtopcoll_section_name($course, $section, true);
            }
            if ((($this->mobiletheme === false) && ($this->tablettheme === false)) || ($this->userisediting)) {
            ...
            if ($this->userisediting) {
                ...
                $o .= html_writer::tag('span',
                    $this->courseformat->get_tabbedtopcoll_section_name($course, $section, false),
    ...
    protected function stealth_section_header($sectionno) {
        ...
        $liattributes = array(
            ...
            'role' => 'region',
            'aria-label' => $this->courseformat->get_tabbedtopcoll_section_name($course, $sectionno, false)
        );
        if (($this->formatresponsive) && ($this->tcsettings['layoutcolumnorientation'] == 2)) { // Horizontal column layout.
            $liattributes['style'] = 'width: ' . $this->tccolumnwidth . '%;';
        }
    ...
    protected function toggle_all() {
        ...
        $o .= html_writer::tag('span', get_string('topcollopened', 'format_tabbedtopcoll'),
            array('class' => 'on ' . $this->tctoggleiconsize, 'id' => 'toggles-all-opened',
            'role' => 'button')
        );
        $o .= html_writer::tag('span', get_string('topcollclosed', 'format_tabbedtopcoll'),
    ...
    protected function display_instructions() {
        ...
        $o .= html_writer::tag('p', get_string('instructions', 'format_tabbedtopcoll'),
