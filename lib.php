...
class format_tabbedtopcoll extends format_base {
    ...
    public function get_section_name($section) {
        ...
        return $this->get_tabbedtopcoll_section_name($course, $section, false);
    }
     ...
     * @return string The section name.
     */
    public function get_tabbedtopcoll_section_name($course, $section, $additional) {
        ...
        if ((string) $thesection->name !== '') {
            ...
            $o = get_string('section0name', 'format_tabbedtopcoll');
        } else {
            if (($tcsettings['layoutstructure'] == 1) || ($tcsettings['layoutstructure'] == 4)) {
                $o = get_string('sectionname', 'format_tabbedtopcoll') . ' ' . $thesection->section;
            } else {
                $o .= $this->get_section_dates($section, $course, $tcsettings);
            }
        }
        ...
        if (($additional == true) && ($thesection->section != 0)) {
            switch ($tcsettings['layoutelement']) {
                ...
                case 4:
                    // The word 'Toggle'.
                    $o .= '<div class="cttoggle"> - ' .get_string('topcolltoggle', 'format_tabbedtopcoll') . '</div>';
                    break;
            }
        }
    ...
    public function get_section_dates($section, $course, $tcsettings) {
        ...
        if ($tcsettings['layoutstructure'] == 5) {
            $day = $this->format_tabbedtopcoll_get_section_day($section, $course);

            $weekday = userdate($day, $dateformat);
            $o = $weekday;
        } else {
            $dates = $this->format_tabbedtopcoll_get_section_dates($section, $course);
    ...
    public function ajax_section_move() {
        ...
        if ($sections = $modinfo->get_section_info_all()) {
            foreach ($sections as $number => $section) {
                $titles[$number] = $this->get_tabbedtopcoll_section_name($course, $section, true);
    ...
    public function section_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($sectionformatoptions['donotshowdate']['label'])) {
            $sectionformatoptionsedit = array(
                'donotshowdate' => array(
                    'label' => new lang_string('donotshowdate', 'format_tabbedtopcoll'),
                    'help' => 'donotshowdate',
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            ...
            $defaulttgfgcolour = get_config('format_tabbedtopcoll', 'defaulttgfgcolour');
            if ($defaulttgfgcolour[0] == '#') {
                $defaulttgfgcolour = substr($defaulttgfgcolour, 1);
            }
            $defaulttgfghvrcolour = get_config('format_tabbedtopcoll', 'defaulttgfghvrcolour');
            if ($defaulttgfghvrcolour[0] == '#') {
                $defaulttgfghvrcolour = substr($defaulttgfghvrcolour, 1);
            }
            $defaulttgbgcolour = get_config('format_tabbedtopcoll', 'defaulttgbgcolour');
            if ($defaulttgbgcolour[0] == '#') {
                $defaulttgbgcolour = substr($defaulttgbgcolour, 1);
            }
            $defaulttgbghvrcolour = get_config('format_tabbedtopcoll', 'defaulttgbghvrcolour');
            ...
            $courseformatoptions = array(
                ...
                'displayinstructions' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultdisplayinstructions'),
                    'type' => PARAM_INT,
                ),
                'layoutelement' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultlayoutelement'),
                    'type' => PARAM_INT,
                ),
                'layoutstructure' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultlayoutstructure'),
                    'type' => PARAM_INT,
                ),
                'layoutcolumns' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultlayoutcolumns'),
                    'type' => PARAM_INT,
                ),
                'layoutcolumnorientation' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultlayoutcolumnorientation'),
                    'type' => PARAM_INT,
                ),
                'togglealignment' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttogglealignment'),
                    'type' => PARAM_INT,
                ),
                'toggleiconposition' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttoggleiconposition'),
                    'type' => PARAM_INT,
                ),
                'toggleiconset' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttoggleiconset'),
                    'type' => PARAM_ALPHA,
                ),
                'onesection' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultonesection'),
                    'type' => PARAM_INT,
                ),
                'toggleallhover' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttoggleallhover'),
                    'type' => PARAM_INT,
                ),
                ...
                'toggleforegroundopacity' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttgfgopacity'),
                    'type' => PARAM_RAW,
                ),
                ...
                'toggleforegroundhoveropacity' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttgbghvropacity'),
                    'type' => PARAM_RAW,
                ),
                ...
                'togglebackgroundopacity' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaulttgbgopacity'),
                    'type' => PARAM_RAW,
                ),
                ...
                'showsectionsummary' => array(
                    'default' => get_config('format_tabbedtopcoll', 'defaultshowsectionsummary'),
                    'type' => PARAM_INT,
                )
            );
        }
        if ($foreditform && !isset($courseformatoptions['displayinstructions']['label'])) {
            ...
            $courseformatoptionsedit = array(
                'numsections' => array(
                    'label' => new lang_string('numbersections', 'format_tabbedtopcoll'),
                    'element_type' => 'select',
                ...
                'displayinstructions' => array(
                    'label' => new lang_string('displayinstructions', 'format_tabbedtopcoll'),
            ...
            if (has_capability('format/tabbedtopcoll:changelayout', $context)) {
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => new lang_string('setlayoutelements', 'format_tabbedtopcoll'),
                    ...
                    'element_attributes' => array( // In insertion order and not numeric for sorting purposes.
                        array(
                            ...
                            1 => new lang_string('setlayout_all', 'format_tabbedtopcoll'),
                            // Toggle word and toggle section x.
                            3 => new lang_string('setlayout_toggle_word_section_x', 'format_tabbedtopcoll'),
                            // Toggle word and section number.
                            2 => new lang_string('setlayout_toggle_word_section_number', 'format_tabbedtopcoll'),
                            // Toggle section x and section number.
                            5 => new lang_string('setlayout_toggle_section_x_section_number', 'format_tabbedtopcoll'),
                            // Toggle word.
                            4 => new lang_string('setlayout_toggle_word', 'format_tabbedtopcoll'),
                            // Toggle section x.
                            8 => new lang_string('setlayout_toggle_section_x', 'format_tabbedtopcoll'),
                            // Section number.
                            6 => new lang_string('setlayout_section_number', 'format_tabbedtopcoll'),
                            // No additions.
                            7 => new lang_string('setlayout_no_additions', 'format_tabbedtopcoll'))
                    )
                );
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => new lang_string('setlayoutstructure', 'format_tabbedtopcoll'),
                    ...
                    'element_attributes' => array(
                        array(
                            // Topic.
                            1 => new lang_string('setlayoutstructuretopic', 'format_tabbedtopcoll'),
                            // Week.
                            2 => new lang_string('setlayoutstructureweek', 'format_tabbedtopcoll'),
                            // Current Week First.
                            3 => new lang_string('setlayoutstructurelatweekfirst', 'format_tabbedtopcoll'),
                            // Current Topic First.
                            4 => new lang_string('setlayoutstructurecurrenttopicfirst', 'format_tabbedtopcoll'),
                            // Day.
                            5 => new lang_string('setlayoutstructureday', 'format_tabbedtopcoll'))
                    )
                );
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => new lang_string('setlayoutcolumns', 'format_tabbedtopcoll'),
                    'help' => 'setlayoutcolumns',
                    'help_component' => 'format_tabbedtopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('one', 'format_tabbedtopcoll'),   // Default.
                              2 => new lang_string('two', 'format_tabbedtopcoll'),   // Two.
                              3 => new lang_string('three', 'format_tabbedtopcoll'), // Three.
                              4 => new lang_string('four', 'format_tabbedtopcoll'))  // Four.
                    )
                );
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => new lang_string('setlayoutcolumnorientation', 'format_tabbedtopcoll'),
                    'help' => 'setlayoutcolumnorientation',
                    'help_component' => 'format_tabbedtopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('columnvertical', 'format_tabbedtopcoll'),
                              2 => new lang_string('columnhorizontal', 'format_tabbedtopcoll')) // Default.
                    )
                );
                $courseformatoptionsedit['toggleiconposition'] = array(
                    'label' => new lang_string('settoggleiconposition', 'format_tabbedtopcoll'),
                    'help' => 'settoggleiconposition',
                    'help_component' => 'format_tabbedtopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('left', 'format_tabbedtopcoll'),   // Left.
                              2 => new lang_string('right', 'format_tabbedtopcoll'))  // Right.
                    )
                );
                $courseformatoptionsedit['onesection'] = array(
                    'label' => new lang_string('onesection', 'format_tabbedtopcoll'),
                    'help' => 'onesection',
                    'help_component' => 'format_tabbedtopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                ...
                $courseformatoptionsedit['showsectionsummary'] = array(
                    'label' => new lang_string('setshowsectionsummary', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaultlayoutelement'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaultlayoutstructure'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaultlayoutcolumns'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaultlayoutcolumnorientation'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleiconposition'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttoggleiconposition'), 'element_type' => 'hidden');
                $courseformatoptionsedit['onesection'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaultonesection'), 'element_type' => 'hidden');
                $courseformatoptionsedit['showsectionsummary'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaultshowsectionsummary'), 'element_type' => 'hidden');
            }

            if (has_capability('format/tabbedtopcoll:changetogglealignment', $context)) {
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => new lang_string('settogglealignment', 'format_tabbedtopcoll'),
                    'help' => 'settogglealignment',
                    'help_component' => 'format_tabbedtopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('left', 'format_tabbedtopcoll'),   // Left.
                              2 => new lang_string('center', 'format_tabbedtopcoll'), // Centre.
                              3 => new lang_string('right', 'format_tabbedtopcoll'))  // Right.
                    )
                );
            } else {
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttogglealignment'), 'element_type' => 'hidden');
            }

            if (has_capability('format/tabbedtopcoll:changetoggleiconset', $context)) {
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => new lang_string('settoggleiconset', 'format_tabbedtopcoll'),
                    'help' => 'settoggleiconset',
                    'help_component' => 'format_tabbedtopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                            'arrow' => new lang_string('arrow', 'format_tabbedtopcoll'),               // Arrow icon set.
                            'bulb' => new lang_string('bulb', 'format_tabbedtopcoll'),                 // Bulb icon set.
                            'cloud' => new lang_string('cloud', 'format_tabbedtopcoll'),               // Cloud icon set.
                            'eye' => new lang_string('eye', 'format_tabbedtopcoll'),                   // Eye icon set.
                            'folder' => new lang_string('folder', 'format_tabbedtopcoll'),             // Folder icon set.
                            'groundsignal' => new lang_string('groundsignal', 'format_tabbedtopcoll'), // Ground signal set.
                            'led' => new lang_string('led', 'format_tabbedtopcoll'),                   // LED icon set.
                            'point' => new lang_string('point', 'format_tabbedtopcoll'),               // Point icon set.
                            'power' => new lang_string('power', 'format_tabbedtopcoll'),               // Power icon set.
                            'radio' => new lang_string('radio', 'format_tabbedtopcoll'),               // Radio icon set.
                            'smiley' => new lang_string('smiley', 'format_tabbedtopcoll'),             // Smiley icon set.
                            'square' => new lang_string('square', 'format_tabbedtopcoll'),             // Square icon set.
                            'sunmoon' => new lang_string('sunmoon', 'format_tabbedtopcoll'),           // Sun / Moon icon set.
                            'switch' => new lang_string('switch', 'format_tabbedtopcoll'))             // Switch icon set.
                    )
                );
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => new lang_string('settoggleallhover', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttoggleiconset'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttoggleallhover'), 'element_type' => 'hidden');
            }
            ...
            if (has_capability('format/tabbedtopcoll:changecolour', $context)) {
                ...
                $courseformatoptionsedit['toggleforegroundcolour'] = array(
                    'label' => new lang_string('settoggleforegroundcolour', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['toggleforegroundopacity'] = array(
                    'label' => new lang_string('settoggleforegroundopacity', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['toggleforegroundhovercolour'] = array(
                    'label' => new lang_string('settoggleforegroundhovercolour', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['toggleforegroundhoveropacity'] = array(
                    'label' => new lang_string('settoggleforegroundhoveropacity', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['togglebackgroundcolour'] = array(
                    'label' => new lang_string('settogglebackgroundcolour', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['togglebackgroundopacity'] = array(
                    'label' => new lang_string('settogglebackgroundopacity', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['togglebackgroundhovercolour'] = array(
                    'label' => new lang_string('settogglebackgroundhovercolour', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['togglebackgroundhoveropacity'] = array(
                    'label' => new lang_string('settogglebackgroundhoveropacity', 'format_tabbedtopcoll'),
                ...
                $courseformatoptionsedit['toggleforegroundopacity'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttgfgopacity'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleforegroundhovercolour'] = array(
                    'label' => $defaulttgfghvrcolour, 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleforegroundhoveopacity'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttgfghvropacity'), 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundcolour'] = array(
                    'label' => $defaulttgbgcolour, 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundopacity'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttgbgopacity'), 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundhovercolour'] = array(
                    'label' => $defaulttgbghvrcolour, 'element_type' => 'hidden');
                $courseformatoptionsedit['togglebackgroundhoveopacity'] = array(
                    'label' => get_config('format_tabbedtopcoll', 'defaulttgbghvropacity'), 'element_type' => 'hidden');
            }
            ...
            $courseformatoptionsedit['readme'] = array(
                    'label' => get_string('readme_title', 'format_tabbedtopcoll'),
                    'element_type' => 'static',
                    'element_attributes' => array(get_string('readme_desc', 'format_tabbedtopcoll', array('url' => $readme)))
                );
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        MoodleQuickForm::registerElementType('tccolourpopup', "$CFG->dirroot/course/format/tabbedtopcoll/js/tc_colourpopup.php",
        ...
        $changelayout = has_capability('format/tabbedtopcoll:changelayout', $context);
        $changecolour = has_capability('format/tabbedtopcoll:changecolour', $context);
        $changetogglealignment = has_capability('format/tabbedtopcoll:changetogglealignment', $context);
        $changetoggleiconset = has_capability('format/tabbedtopcoll:changetoggleiconset', $context);
        $resetall = is_siteadmin($USER); // Site admins only.

        $elements[] = $mform->addElement('header', 'ctreset', get_string('ctreset', 'format_tabbedtopcoll'));
        $mform->addHelpButton('ctreset', 'ctreset', 'format_tabbedtopcoll', '', true);

        $resetelements = array();
        $checkboxname = get_string('resetdisplayinstructions', 'format_tabbedtopcoll').
            $OUTPUT->help_icon('resetdisplayinstructions', 'format_tabbedtopcoll');
        $resetelements[] =& $mform->createElement('checkbox', 'resetdisplayinstructions', '', $checkboxname);

        if ($changelayout) {
            $checkboxname = get_string('resetlayout', 'format_tabbedtopcoll').$OUTPUT->help_icon('resetlayout', 'format_tabbedtopcoll');
            $resetelements[] =& $mform->createElement('checkbox', 'resetlayout', '', $checkboxname);
        }

        if ($changecolour) {
            $checkboxname = get_string('resetcolour', 'format_tabbedtopcoll').$OUTPUT->help_icon('resetcolour', 'format_tabbedtopcoll');
            $resetelements[] =& $mform->createElement('checkbox', 'resetcolour', '', $checkboxname);
        }

        if ($changetogglealignment) {
            $checkboxname = get_string('resettogglealignment', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resettogglealignment', 'format_tabbedtopcoll');
            $resetelements[] =& $mform->createElement('checkbox', 'resettogglealignment', '', $checkboxname);
        }

        if ($changetoggleiconset) {
            $checkboxname = get_string('resettoggleiconset', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resettoggleiconset', 'format_tabbedtopcoll');
            $resetelements[] =& $mform->createElement('checkbox', 'resettoggleiconset', '', $checkboxname);
        }
        $elements[] = $mform->addGroup($resetelements, 'resetgroup', get_string('resetgrp', 'format_tabbedtopcoll'), null, false);

        if ($resetall) {
            $resetallelements = array();

            $checkboxname = get_string('resetalldisplayinstructions', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resetalldisplayinstructions', 'format_tabbedtopcoll');
            $resetallelements[] =& $mform->createElement('checkbox', 'resetalldisplayinstructions', '', $checkboxname);

            $checkboxname = get_string('resetalllayout', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resetalllayout', 'format_tabbedtopcoll');
            $resetallelements[] =& $mform->createElement('checkbox', 'resetalllayout', '', $checkboxname);

            $checkboxname = get_string('resetallcolour', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resetallcolour', 'format_tabbedtopcoll');
            $resetallelements[] =& $mform->createElement('checkbox', 'resetallcolour', '', $checkboxname);

            $checkboxname = get_string('resetalltogglealignment', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resetalltogglealignment', 'format_tabbedtopcoll');
            $resetallelements[] =& $mform->createElement('checkbox', 'resetalltogglealignment', '', $checkboxname);

            $checkboxname = get_string('resetalltoggleiconset', 'format_tabbedtopcoll').
                $OUTPUT->help_icon('resetalltoggleiconset', 'format_tabbedtopcoll');
            ...
            $elements[] = $mform->addGroup($resetallelements, 'resetallgroup',
                get_string('resetallgrp', 'format_tabbedtopcoll'), null, false);
        }
    ...
    public function edit_form_validation($data, $files, $errors) {
        ...
        if ($this->validate_colour($data['toggleforegroundcolour']) === false) {
            $retr['toggleforegroundcolour'] = get_string('colourrule', 'format_tabbedtopcoll');
        }
        if ($this->validate_colour($data['toggleforegroundhovercolour']) === false) {
            $retr['toggleforegroundhovercolour'] = get_string('colourrule', 'format_tabbedtopcoll');
        }
        if ($this->validate_colour($data['togglebackgroundcolour']) === false) {
            $retr['togglebackgroundcolour'] = get_string('colourrule', 'format_tabbedtopcoll');
        }
        if ($this->validate_colour($data['togglebackgroundhovercolour']) === false) {
            $retr['togglebackgroundhovercolour'] = get_string('colourrule', 'format_tabbedtopcoll');
        }
    ...
    public function is_section_current($section) {
        ...
        if (($tcsettings['layoutstructure'] == 2) || ($tcsettings['layoutstructure'] == 3)) {
            ...
            $timenow = time();
            $dates = $this->format_tabbedtopcoll_get_section_dates($section, $this->get_course());
            ...
            $timenow = time();
            $day = $this->format_tabbedtopcoll_get_section_day($section, $this->get_course());
            $onedayseconds = 86400;
     ...
     */
    private function format_tabbedtopcoll_get_section_dates($section, $course) {
     ...
     */
    private function format_tabbedtopcoll_get_section_day($section, $course) {
        $onedayseconds = 86400;
        /* Hack alert. We add 2 hours to avoid possible DST problems. (e.g. we go into daylight
           savings and the date changes. */
        $startdate = $course->startdate + 7200;
     ...
     */
    public function reset_tabbedtopcoll_setting($courseid, $displayinstructions, $layout, $colour, $togglealignment, $toggleiconset) {
        global $DB, $USER;
        ...
        if ($displayinstructions && $resetallifall) {
            $updatedata['displayinstructions'] = get_config('format_tabbedtopcoll', 'defaultdisplayinstructions');
            $updatedisplayinstructions = true;
        }
        if ($layout && has_capability('format/tabbedtopcoll:changelayout', $context) && $resetallifall) {
            $updatedata['layoutelement'] = get_config('format_tabbedtopcoll', 'defaultlayoutelement');
            $updatedata['layoutstructure'] = get_config('format_tabbedtopcoll', 'defaultlayoutstructure');
            $updatedata['layoutcolumns'] = get_config('format_tabbedtopcoll', 'defaultlayoutcolumns');
            $updatedata['layoutcolumnorientation'] = get_config('format_tabbedtopcoll', 'defaultlayoutcolumnorientation');
            $updatedata['toggleiconposition'] = get_config('format_tabbedtopcoll', 'defaulttoggleiconposition');
            $updatedata['onesection'] = get_config('format_tabbedtopcoll', 'defaultonesection');
            $updatedata['showsectionsummary'] = get_config('format_tabbedtopcoll', 'defaultshowsectionsummary');
            $updatelayout = true;
        }
        if ($togglealignment && has_capability('format/tabbedtopcoll:changetogglealignment', $context) && $resetallifall) {
            $updatedata['togglealignment'] = get_config('format_tabbedtopcoll', 'defaulttogglealignment');
            $updatetogglealignment = true;
        }
        if ($colour && has_capability('format/tabbedtopcoll:changecolour', $context) && $resetallifall) {
            $updatedata['toggleforegroundcolour'] = get_config('format_tabbedtopcoll', 'defaulttgfgcolour');
            $updatedata['toggleforegroundopacity'] = get_config('format_tabbedtopcoll', 'defaulttgfgopacity');
            $updatedata['toggleforegroundhovercolour'] = get_config('format_tabbedtopcoll', 'defaulttgfghvrcolour');
            $updatedata['toggleforegroundhoveropacity'] = get_config('format_tabbedtopcoll', 'defaulttgfghvropacity');
            $updatedata['togglebackgroundcolour'] = get_config('format_tabbedtopcoll', 'defaulttgbgcolour');
            $updatedata['togglebackgroundopacity'] = get_config('format_tabbedtopcoll', 'defaulttgbgopacity');
            $updatedata['togglebackgroundhovercolour'] = get_config('format_tabbedtopcoll', 'defaulttgbghvrcolour');
            $updatedata['togglebackgroundhoveropacity'] = get_config('format_tabbedtopcoll', 'defaulttgbghvropacity');
            $updatecolour = true;
        }
        if ($toggleiconset && has_capability('format/tabbedtopcoll:changetoggleiconset', $context) && $resetallifall) {
            $updatedata['toggleiconset'] = get_config('format_tabbedtopcoll', 'defaulttoggleiconset');
            $updatedata['toggleallhover'] = get_config('format_tabbedtopcoll', 'defaulttoggleallhover');
     ...
     * in the old 'format_tabbedtopcoll_settings' table.
     * @param int $courseid If not 0, then a specific course to reset.
     ...
     */
    public function restore_tabbedtopcoll_setting($courseid, $layoutelement, $layoutstructure, $layoutcolumns, $tgfgcolour,
        ...
        $lco = get_config('format_tabbedtopcoll', 'defaultlayoutcolumnorientation');
        if (empty($lco)) {
            ...
            $data['displayinstructions'] = get_config('format_tabbedtopcoll', 'defaultdisplayinstructions');
            $data['layoutcolumnorientation'] = get_config('format_tabbedtopcoll', 'defaultlayoutcolumnorientation');
            $data['showsectionsummary'] = get_config('format_tabbedtopcoll', 'defaultshowsectionsummary');
            $data['togglealignment'] = get_config('format_tabbedtopcoll', 'defaulttogglealignment');
            $data['toggleallhover'] = get_config('format_tabbedtopcoll', 'defaulttoggleallhover');
            $data['toggleiconposition'] = get_config('format_tabbedtopcoll', 'defaulttoggleiconposition');
            $data['toggleiconset'] = get_config('format_tabbedtopcoll', 'defaulttoggleiconset');
        }
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_tabbedtopcoll');
        }
        if (empty($editlabel)) {
            $course = $this->get_course();
            $title = $this->get_tabbedtopcoll_section_name($course, $section, false);
            $editlabel = new lang_string('newsectionname', 'format_tabbedtopcoll', $title);
        }
 ...
 */
function callback_tabbedtopcoll_definition() {
    return get_string('sectionname', 'format_tabbedtopcoll');
}
